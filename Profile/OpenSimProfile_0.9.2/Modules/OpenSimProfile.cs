...
[assembly: Addin("OpenSimProfileModule", "1.1")]
[assembly: AddinDependency("OpenSim.Region.Framework", OpenSim.VersionInfo.VersionNumber)]


namespace OpenSim.Modules.OpenSimProfile
{
	[Extension(Path = "/OpenSim/RegionModules", NodeName = "RegionModule", Id = "OpenSimProfileModule")]

    //public class OpenSimProfileModule : IProfileModule, ISharedRegionModule
    public class OpenSimProfileModule : IProfileModule, INonSharedRegionModule
	{
		...
		public string Name
		{
			get { return "OpenSimProfileModule"; }
		}
		...
		{
			//m_log.Info("[OSPROFILE] Initialise");
			if (!m_Enabled) return;
			...
			if (m_Scenes.Count==0) { 	// First time
				...
				string module_name = profileConfig.GetString("Module", "None");
                if (module_name!=Name) {
					m_log.InfoFormat("[OSPROFILE] OpenSimProfile module is disable. Module name is mismacth. ({0})", module_name);
					m_Enabled = false;
					return;
				}
				//
				...
				if (m_ProfileServer!="") {
					m_log.Info("[OSPROFILE] OpenSimProfile module is activated");
				}
				else {
					m_log.Info("[OSPROFILE] OpenSimProfile module is disable. There is no ProfileURL");
					m_Enabled = false;
					return;
				}
            ...
            /*
			if (connectString!="") {
				m_log.Info("[OSPROFILE] Initialising DB");
				OpenSimProfilesData profdata = new OpenSimProfilesData();
				profdata.init(connectString);
			}*/
		...
		{
			//m_log.Info("[OSPROFILE] AddRegion");
			if (!m_Enabled) return;
		...
		private Hashtable GenericXMLRPCRequest(Hashtable ReqParams, string method)
		{
			...
			{
				m_log.ErrorFormat("[OSPROFILE]: Unable to connect to Profile Server (WEB): {0}, Exception: {1}", m_ProfileServer, ex);
			...
			{
				m_log.ErrorFormat("[OSPROFILE]: Unable to connect to Profile Server (SOCKET): {0}, Method: {1}, Params: {2}, Exception: {3}", 
			...
			{
				m_log.ErrorFormat("[OSPROFILE]: Unable to connect to Profile Server (XML): {0}, Method: {1}, Params: {2}, Exception: {3}", 
										...
										int queryclassifiedPrice, IClientAPI remoteClient)
		{
			...
			ReqHash["name"] 			= Convert.ToBase64String(Enc.GetBytes(queryName));
			ReqHash["description"] 		= Convert.ToBase64String(Enc.GetBytes(queryDescription));
		...
		// Update Picks
		public void PickInfoUpdate(IClientAPI remoteClient, UUID pickID, UUID creatorID, 
					bool topPick, string name, string desc, UUID snapshotID, int sortOrder, bool enabled)
		{
			Hashtable ReqHash = new Hashtable();
			...
			ReqHash["name"] 		= Convert.ToBase64String(Enc.GetBytes(name));
			ReqHash["desc"] 		= Convert.ToBase64String(Enc.GetBytes(desc));
		...
		public void HandleAvatarNotesRequest(Object sender, string method, List<String> args) 
		{
			...
			string notes = "";
			if (d.ContainsKey("notes")) notes = d["notes"].ToString();
		...
		public void AvatarNotesUpdate(IClientAPI remoteClient, UUID queryTargetID, string queryNotes)
		{
			...
			ReqHash["notes"]	 = Convert.ToBase64String(Enc.GetBytes(queryNotes));
		...
		public void UpdateAvatarProperties(IClientAPI remoteClient, UserProfileData newProfile)
		{
			if (remoteClient.AgentId==newProfile.ID)
			{
				...
				ReqHash["AboutText"] 	  = Convert.ToBase64String(Enc.GetBytes(aboutText));
				ReqHash["FirstLifeText"]  = Convert.ToBase64String(Enc.GetBytes(firstLifeText));
		...
		*/
		...
		public void AvatarPropertiesUpdate(IClientAPI remoteClient, UserProfileProperties newProfile)
		{
			...
			{
				string profileUrl 	  = newProfile.WebUrl;
				string image 		  = newProfile.ImageId.ToString();
				string firstLifeImage = newProfile.FirstLifeImageId.ToString();
				string aboutText 	  = newProfile.AboutText;
				string firstLifeText  = newProfile.FirstLifeText;
				string partner	  	  = newProfile.PartnerId.ToString();

				int flags = 0;
            	flags |= newProfile.PublishProfile ? 1 : 0;
            	flags |= newProfile.PublishMature  ? 2 : 0;
				string userFlags	  = flags.ToString();
		...
		public void AvatarInterestsUpdate(IClientAPI remoteClient, uint wantmask, string wanttext, uint skillsmask, string skillstext, string languages)
		{
			...
			ReqHash["WantToText"] 	 = Convert.ToBase64String(Enc.GetBytes(wanttext));
			ReqHash["SkillsMask"] 	 = skillsmask.ToString();
			ReqHash["SkillsText"] 	 = Convert.ToBase64String(Enc.GetBytes(skillstext));
